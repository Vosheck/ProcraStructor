    ...
    internal class ViewModelModelConverter
    {
        ...
        public ISolutionModel ToModel(ISolution solutionRoot)
        {
            ...
            foreach (var item in items.Select(i => i.Node))
            {
                ...
                if (item.Parent == null)
                {
                    solutionModel.AddSolutionRootItem(item.DisplayName, item.GetId(),item.IsChecked);
                ...
                else
                {
                    ...
                    //Custom
                    modelNewChild.IsChecked = item.IsChecked;
                                    ...
                                    ,ISolution solutionRoot)
        {
            ...
            IEnumerable<IItemModel> itemsSelected = items.Select(i => i.Node);
            ...
            , IItemModel item)
        {
            ...
            switch (item.ItemType)
            {
                case SolutionModelItemType.File:
                    modelNewChild = solutionModel.AddChild(item.DisplayName, SolutionItemType.SubTask, parent);
                    break;

                case SolutionModelItemType.Folder:
                    modelNewChild = solutionModel.AddChild(item.DisplayName, SolutionItemType.TopTask, parent);
                    break;
